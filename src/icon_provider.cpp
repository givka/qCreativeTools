...
const QStringList IconProvider::ImageTypes = { "bmp", "gif", "jpg", "jpeg", "png", "pbm", "pgm", "ppm", "xbm", "xpm" };
...
QIcon IconProvider::icon(const QFileInfo &info) const
{
    ...
    if (ImageTypes.contains(info.suffix().toLower()))
    {
        ...
        pixMap = pixMap.scaled(Size, Size, Qt::KeepAspectRatio, Qt::TransformationMode::SmoothTransformation);
        return QIcon(pixMap);
    }
